...
export function AppZ80Opcodes() {
  return(
    <div className="flex flex-auto h-full relative mt-0">
          ...
          <Collapse.Content>
            <Table compact={false} zebra={true} width="100%">
              ...
              <Table.Body>
                ...
                <Table.Row>{[<span>LD r, n</span>, <span>r = n</span>, <span>r: A, B, C, D, E, H, L; n: 8-bit value</span>]}</Table.Row>
        ...
        <Collapse checkbox={true} icon="arrow">
            ...
            <Collapse.Content>
              <Table compact={false} zebra={true} width="100%">
                ...
                <Table.Body>
                  <Table.Row>{[<span>LD dd, nn</span>, <span>dd = nn</span>, <span>dd: BC, DE, HL, SP, IX, IY; nn: 16-bit value</span>]}</Table.Row>
                  <Table.Row>{[<span>LD dd, (nn)</span>, <span>low(dd) = nn[0], high(dd) = nn[1]</span>, <span>dd: BC, DE, HL, SP, IX, IY; nn: 16-bit value</span>]}</Table.Row>
                  <Table.Row>{[<span>LD (nn), dd</span>, <span>nn[0] = low(dd), nn[1] = high(dd)</span>, <span>dd: BC, DE, HL, SP, IX, IY; nn: 16-bit value</span>]}</Table.Row>
      ...
      <h1 className="font-bold mb-2 text-white">Pseudo-Instructions</h1>
    </div>
    </div>
  );
}
