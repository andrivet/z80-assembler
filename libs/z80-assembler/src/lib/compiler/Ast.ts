 ...
 */
abstract class Value8Base implements AstBase {
  private readonly position: Position;
  ...
  protected abstract adjust(v: number): number;
  ...
  generate(pc: Address): bytes {
    ...
    // If the value does not fit into 8 bits, we have a compilation error.
...
class Value8 extends Value8Base {
  protected adjust(v: number) { return v; }
}

class Value8Plus1 extends Value8Base {
  protected adjust(v: number) { return v + 1; }
}

class ValueNeg8 extends Value8Base {
  protected adjust(v: number) { return -v; }
}

class ValueNeg8Plus1 extends Value8Base {
  protected adjust(v: number) { return -v + 1; }
}
...
export function index(offset: {pos: PosInfo, s: string, d: Expression} | null): AstElement {
  return !offset
    ? 0
    : offset.s === '-'
    ? new ValueNeg8(offset.pos, offset.d)
    : new Value8(offset.pos, offset.d);
}

/**
 * Returns an AST element that represents an 8-bit signed offset for IX or IY, plus 1.
 * @offset The offset for IX or IY with:
 * pos The position of the element in the source.
 * s The sign of the value.
 */
export function index_1(offset: {pos: PosInfo, s: string, d: Expression} | null): AstElement {
  return !offset
    ? 1
    : offset.s === '-'
    ? new ValueNeg8Plus1(offset.pos, offset.d)
    : new Value8Plus1(offset.pos, offset.d);
}
